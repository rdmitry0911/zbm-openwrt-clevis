...
for _zbm_runtime_env in /etc/zfsbootmenu-kcl.env /run/zbm-runtime/kcl.env; do
  [ -r "${_zbm_runtime_env}" ] && source "${_zbm_runtime_env}" >/dev/null 2>&1
done
unset _zbm_runtime_env
...
mount_zfs() {
  ...
  if awk -v zfsfs="${fs}" -v mp="${mnt}" \
    '$1 == zfsfs && $2 == mp { found=1 } END { exit !found }' \
    /proc/mounts 2>/dev/null
  then
    zdebug "${fs} already mounted at ${mnt}"
    echo "${mnt}"
    return 0
  fi
...
# arg1: mounted boot environment root
# arg2: root filesystem of the boot environment
# prints: nothing
# returns: 0 if a separate /boot dataset was mounted, 1 otherwise

mount_be_boot() {
  local mnt fs leaf bootfs ret

  mnt="${1}"
  fs="${2}"

  if [ -z "${mnt}" ] || [ -z "${fs}" ]; then
    zerror "mountpoint or filesystem is undefined"
    return 1
  fi
  ...
  leaf="${fs##*/}"
  bootfs="$(
    zfs list -H -o name,mountpoint 2>/dev/null | awk -v leaf="${leaf}" '
      $2 == "/boot" && $1 ~ ("/" leaf "$") { print $1; exit }
    '
  )"

  if [ -z "${bootfs}" ] || [ "${bootfs}" = "${fs}" ]; then
    zdebug "no separate /boot dataset found for ${fs}"
    return 1
  fi

  if awk -v zfsfs="${bootfs}" -v mp="${mnt}/boot" \
    '$1 == zfsfs && $2 == mp { found=1 } END { exit !found }' \
    /proc/mounts 2>/dev/null
  then
    zdebug "${bootfs} already mounted at ${mnt}/boot"
    return 0
  fi

  zdebug "mounting separate boot dataset ${bootfs} at ${mnt}/boot"
  if [ "$(zfs get -H -o value mountpoint "${bootfs}")" = "legacy" ] || is_snapshot "${bootfs}" ; then
    mount -o ro -t zfs "${bootfs}" "${mnt}/boot"
    ret=$?
  else
    mount -o zfsutil,ro -t zfs "${bootfs}" "${mnt}/boot"
    ret=$?
  fi

  if [ "${ret}" -ne 0 ]; then
    zwarn "failed to mount separate /boot dataset ${bootfs} for ${fs}"
    return 1
  fi
...
# arg1: mounted boot environment root
# prints: nothing
# returns: 0 on successful cleanup, 1 otherwise
...
umount_be() {
  ...
  if [ -z "${mnt}" ]; then
    zerror "mountpoint is undefined"
    return 1
  fi

  if awk -v mp="${mnt}/boot" '$2 == mp { found=1 } END { exit !found }' /proc/mounts 2>/dev/null; then
    umount "${mnt}/boot" || ret=1
  fi

  if awk -v mp="${mnt}" '$2 == mp { found=1 } END { exit !found }' /proc/mounts 2>/dev/null; then
    umount "${mnt}" || ret=1
  fi
...
kexec_kernel() {
  ...
  mount_be_boot "${mnt}" "${fs}" >/dev/null 2>&1 || true

  cli_args="$( load_be_cmdline "${fs}" )"
  root_prefix="$( find_root_prefix "${fs}" "${mnt}" )"

  if ! output="$( kexec -a --reuse-video-type -l "${mnt}${kernel}" \
...
load_be_cmdline() {
  ...
  if [ -n "${ZBM_TARGET_KCL_OVERRIDE:-}" ]; then
    zdebug "using ZBM_TARGET_KCL_OVERRIDE as command line for ${fs}"
    args="$(kcl_tokenize <<< "${ZBM_TARGET_KCL_OVERRIDE}")"
  else
  ...
  if [ -n "${ZBM_TARGET_FBCON_MAP:-}" ]; then
    adds+=( "fbcon=map:${ZBM_TARGET_FBCON_MAP}" )
  fi

  if [ -n "${ZBM_TARGET_KCL_APPEND:-}" ]; then
    while IFS= read -r _zbm_target_arg; do
      [ -n "${_zbm_target_arg}" ] && adds+=( "${_zbm_target_arg}" )
    done <<< "$(kcl_tokenize <<< "${ZBM_TARGET_KCL_APPEND}")"
    unset _zbm_target_arg
  fi
